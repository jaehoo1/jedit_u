...
class wordCountWindow(QDialog):
    def __init__(self, parent):
        ...
        self.pushButton_savetocsv.clicked.connect(self.savetocsvFunction)
    ...
    def savetocsvFunction(self):
        R = self.tableWidget.rowCount()
        df = pandas.DataFrame(columns=['단어', '횟수'])
        #df = pandas.DataFrame()
        df.set_index('단어')

        for i in range(R):
             df.loc[i] = [self.tableWidget.item(i, 0).text(), self.tableWidget.item(i, 1).text()]
             #df.loc[self.tableWidget.item(i, 0).text()] = [self.tableWidget.item(i, 1).text()]
                
        df.to_csv(os.path.dirname(os.path.realpath(__file__))+'/wordCount.csv')
        msgBox = QMessageBox()
        msgBox.setText(os.path.dirname(os.path.realpath(__file__))+'/wordCount.csv' + '에 파일을 저장했습니다.')
        msgBox.addButton('확인', QMessageBox.NoRole)
        msgBox.exec_()


class korToEngWindow(QDialog):
    def __init__(self, parent):
        super(korToEngWindow, self).__init__(parent)
        uic.loadUi(os.path.dirname(os.path.realpath(__file__)) + "/korToEng.ui", self)
        self.show()

        cursor = parent.plainTextEdit.textCursor()
        string = parent.plainTextEdit.toPlainText()[cursor.selectionStart():cursor.selectionEnd()]
        self.plainTextEditKor.setPlainText(string)

        self.pushButton_kortoeng.clicked.connect(self.korToEng)

    def korToEng(self):
        string = self.plainTextEditKor.toPlainText()
        flag = False
        for i in string:
            if i != ' ' and i != '\n':
                flag = True
        ...
        self.plainTextEditEng.setPlainText(translator.translate(self.plainTextEditKor.toPlainText()).text)


class engToKorWindow(QDialog):
    def __init__(self, parent):
        super(engToKorWindow, self).__init__(parent)
        uic.loadUi(os.path.dirname(os.path.realpath(__file__)) + "/engToKor.ui", self)
        self.show()

        cursor = parent.plainTextEdit.textCursor()
        string = parent.plainTextEdit.toPlainText()[cursor.selectionStart():cursor.selectionEnd()]
        self.plainTextEditEng.setPlainText(string)

        self.pushButton_engtokor.clicked.connect(self.engToKor)

    def engToKor(self):
        string = self.plainTextEditEng.toPlainText()
        flag = False
        for i in string:
            if i != ' ' and i != '\n':
                flag = True
        ...
        if not flag:
            return
        translator = Translator()
        self.plainTextEditKor.setPlainText(translator.translate(self.plainTextEditEng.toPlainText(), dest='ko').text)


class spellCheckWindow(QDialog):
    def __init__(self, parent):
        super(spellCheckWindow, self).__init__(parent)
        uic.loadUi(os.path.dirname(os.path.realpath(__file__)) + "/spellCheck.ui", self)
        self.show()

        cursor = parent.plainTextEdit.textCursor()
        string = parent.plainTextEdit.toPlainText()[cursor.selectionStart():cursor.selectionEnd()]
        self.plainTextEditSpell.setPlainText(string)

        self.pushButton_spellcheck.clicked.connect(self.spellCheck)

    def spellCheck(self):
        string = self.plainTextEditSpell.toPlainText()
        spelled_string = spell_checker.check(string)
        self.plainTextEditCheck.setPlainText(spelled_string.checked)


class sellInfoWindow(QDialog):
    def __init__(self, parent):
        super(sellInfoWindow, self).__init__(parent)
        uic.loadUi(os.path.dirname(os.path.realpath(__file__)) + "/sellInfo.ui", self)
        self.show()
        ...
        self.tableWidget.setRowCount(loop)
        self.tableWidget.setColumnCount(3)
        self.tableWidget.setHorizontalHeaderLabels(['일시', '금액', '누적합'])
        for i in range(loop):
            prefix_sum += int(moneylist[i])
            self.tableWidget.setItem(i, 0, QTableWidgetItem(timelist[i]))
            self.tableWidget.setItem(i, 1, QTableWidgetItem(moneylist[i]))
            self.tableWidget.setItem(i, 2, QTableWidgetItem(str(prefix_sum)))

    def savetocsvFunction(self):
        R = self.tableWidget.rowCount()
        df = pandas.DataFrame(columns=['일시', '금액', '누적합'])
        df.set_index('일시')

        for i in range(R):
             df.loc[i] = [self.tableWidget.item(i, 0).text(), self.tableWidget.item(i, 1).text(), self.tableWidget.item(i, 2).text()]
             #df.loc[self.tableWidget.item(i, 0).text()] = [self.tableWidget.item(i, 1).text()]
                
        df.to_csv(os.path.dirname(os.path.realpath(__file__))+'/sellInfo.csv')
        msgBox = QMessageBox()
        msgBox.setText(os.path.dirname(os.path.realpath(__file__))+'/sellInfo.csv' + '에 파일을 저장했습니다.')
        msgBox.addButton('확인', QMessageBox.NoRole)
        msgBox.exec_()
...
class WindowClass(QMainWindow, form_class):
    def __init__(self):
        ...
        self.action_kor_to_eng.triggered.connect(self.korToEngFunction)
        self.action_eng_to_kor.triggered.connect(self.engToKorFunction)
        self.action_spell_check.triggered.connect(self.spellCheckFunction)

        self.action_POS.triggered.connect(self.POSFunction)
        self.action_sell_info.triggered.connect(self.sellInfoFunction)
    ...
    def mailFunction(self):
        recipient = 'jaehoo8936@naver.com'
        title = 'jedit Feedback'
        os.system("thunderbird -compose to=" + recipient + ",subject=" + quote(title))
    ...
    def statusMessage(self):
        ...
        result += "Ln " + str(self.ln) + ", " + "Col " + str(self.col) + "          " + str((self.zoom+10)*10) + "%" + "          Ubuntu(Linux)"
        return result
...
#            app.setFont(font)
    ...
    def korToEngFunction(self):
        korToEngWindow(self)

    def engToKorFunction(self):
        engToKorWindow(self)

    def spellCheckFunction(self):
        spellCheckWindow(self)

    def POSFunction(self):
        os.system("./POS.py")

    def sellInfoFunction(self):
        sellInfoWindow(self)
